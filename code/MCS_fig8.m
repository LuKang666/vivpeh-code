...
Eq1 = zeros(length(delta), length(sigma));
Ep1 = zeros(length(delta), length(sigma));
Eq1p1 = zeros(length(delta), length(sigma));
EV = zeros(length(delta), length(sigma));
EP = zeros(length(delta), length(sigma));

for ii = 1 : length(delta)
    disp(['ii: ', num2str(ii), ' / ', num2str(length(delta))]);
    sysn = @(t, x, w)[x(3); x(4);
                      -om1 * om1 * x(1) - zeta * x(3) + mu * oms(ii) * oms(ii) * x(2) + par(ii) * x(2) * w(1);
    ...
    for jj = 1 : length(sigma)
        disp(['jj: ', num2str(jj), ' / ', num2str(length(sigma))]);
        %% 蒙特卡罗模拟
        for k = 1:Nsamp
            disp([num2str(k), ' / ', num2str(Nsamp)]);
            noise = whitenoise(dt, timt + 1, sigma(jj), 'WhiteNoise');
            ...
            Y(:, 3) = mod(anglexy(om1 * x(:, 1), -x(:, 3)) - anglexy(oms(ii) * x(:, 2), -x(:, 4)) + pi, 2 * pi) - pi; % 相角差
        ...
        XV = YY(:, 1:4); %提取出稳态响应
        points = 60;
        [q1s, pq1s] = getpdf_1(XV(:, 1), [], points);
        [p1s, pp1s] = getpdf_1(XV(:, 3), [], points);
        [~, ~, pq1p1s] = getpdf2(XV(:, [1, 3]), [], [], [points, points]);
        intFun_q1 = q1s .^ 2 .* pq1s;
        intFun_p1 = p1s .^ 2 .* pp1s;
        intFun_q1p1 = q1s .* p1s .* pq1p1s;
        Eq1(ii, jj) = trapz(q1s, intFun_q1);
        Ep1(ii, jj) = trapz(p1s, intFun_p1);
        Eq1p1(ii, jj) = trapz(p1s, trapz(q1s, intFun_q1p1), 2);
        EV(ii, jj) = (omn ^ 2 / eta / (beta ^ 2 + omn ^ 2)) ^ 2 * Eq1(ii, jj) + (beta / eta / (beta ^ 2 + omn ^ 2)) ^ 2 * Ep1(ii, jj) + (2 * beta * omn ^ 2 / eta ^ 2 / (beta ^ 2 + omn ^ 2) ^ 2) * Eq1p1(ii, jj);
        EP(ii, jj) = eta * beta_nondim * EV(ii, jj);
    end
end
...
savePath = strcat(fileDir, 'Fig', num2str(89), '-', num2str(1), '.mat');
save(savePath, 'delta', 'Eq1', 'Ep1', 'EV', 'EP');
