...
% S_0 = S_w(0); S_1 = S_w(om1); S_2 = S_w(2*om1);

S_0 = sigma / 2 / pi; S_1 = sigma / 2 / pi; S_2 = sigma / 2 / pi;

ff = U0^2 * pa^2 / (om1^2 * len^2 * (2*hp + hs)^2 * pb^2);
a1 = @(at) -zeta*at(1) - om1*mu*(1+2*delta)*sqrt(at(1)*at(2))*sin(at(3)) + 18*pi*ff*at(2)*(S_0+S_2);
a2 = @(at) (1 + delta) * alpha * om1 * at(2) - 1.5 * gamma*(1-delta)*at(2)^2/om1 + om1*lambda*sqrt(at(1)*at(2))*cos(at(3));
a3 = @(at) -delta*om1 -0.5*lambda*om1*sin(at(3))*sqrt(at(1)/at(2)) - 0.5*mu*(1+2*delta)*om1*cos(at(3))*sqrt(at(2)/at(1)) - (9*ff*at(2)/at(1))*sin(2*at(3))*S_0;
b11 = @(at) 36*pi*ff*at(1)*at(2)*(2*sin(at(3))^2*S_0+S_2);
b13 = @(at) 18*pi*ff*at(2)*sin(2*at(3))*S_0;
b33 = @(at) (9*pi*ff*at(2)/at(1))*(2*cos(at(3))^2*S_0 + S_2);


drift = {a1;a2;a3}; %漂移系数矩阵
diffu = {b11, "0", b13; "0", "0", "0"; "0", "0", b33}; %扩散系数矩阵
% domain = [0.001, 3; 170, 220; -1.4, 0]; %离散域
domain_H = [0.001, 1.0; 170, 190; -1.4, -0.8];
% domain_H = [0.001, 2.0; 1, 380; -pi, pi];
% domain_H = [0.0038, 1.5016; 1.0154, 367.1; -pi, pi];
class = {'zero', 'zero'; 'zero', 'zero'; 'period', 'period'}; %free是自由边界,period是周期边界,zero是零值边界
% NH = [30, 30, 30]; %输入求解域各维划分份数，两边界上的点也编号，比如分成10份，那么10-1共9个区间
NH = [60, 23, 54];
...
[t, pv] = ode45(sys, [0, 10], p0); % Transient state at enough time
figure;
plot(t, pv(:, end));
...
[H, pH, pHH] = showpxpxy(p, NH, domain_H, C, [1, 2, 3], [1, 2; 1, 3; 2, 3]);
figure;
subplot(1,3,1); surf(H{1},H{2},pHH{1}');
subplot(1,3,2); surf(H{1},H{3},pHH{2}');
subplot(1,3,3); surf(H{2},H{3},pHH{3}');

figure;
subplot(1, 3, 1); plot(H{1}, pH{1}, 'k-');
subplot(1, 3, 2); plot(H{2}, pH{2}, 'k-');
subplot(1, 3, 3); plot(H{3}, pH{3}, 'k-');
...
save(savePath, 'H', 'pH', 'pHH', 'drift', 'diffu', 'domain_H', 'class', 'NH', 'p', 'C');
